...
public static class ImageThemingUtilities
{
    ...
    public static void ThemePixelWithExtraContrast(ref byte r, ref byte g, ref byte b, HslColor background)
    {
...
public struct HslColor
{
    ...
    {
        var q = Luminosity < 0.5 ? Luminosity * (MaxSaturation + Saturation) : Luminosity + Saturation - Luminosity * Saturation;
        var p = 2.0 * Luminosity - q;
        var num = Hue / MaxHue;
        var tC1 = ModOne(num + 1.0 / 3.0);
        var tC2 = num;
        var tC3 = ModOne(num - 1.0 / 3.0);
        return ((byte)(Alpha * byte.MaxValue), (byte)(ComputeRgbComponent(p, q, tC1) * byte.MaxValue), (byte)(ComputeRgbComponent(p, q, tC2) * byte.MaxValue), (byte)(ComputeRgbComponent(p, q, tC3) * byte.MaxValue));
    }
...
internal class ContrastEvaluator
{
    ...
    {
        return (int)Alpha(i) < (int)_minimumContrastEvaluationAlpha ||
               IsCloseToBackground(GetColor(i));
    }
    ...
    {
        return Math.Abs(color.R - _background.R) <= 2 &&
               Math.Abs(color.G - _background.G) <= 2 &&
               Math.Abs(color.B - _background.B) <= 2;
    }
    ...
    private bool IsAntiAliasingSequence(Color[] pixels)
    {
        ...
        if (!IsCloseToBackground(pixels[0]) || IsCloseToBackground(pixels[1]))
            return false;
        double num = _background.ContrastWith(pixels[1]);
...
internal class PixelBuffer
{
    ...
    public void AlphaBlendWithBackground(Color background)
    {
        ...
        {
            Color color = background.BlendWith(GetColor(i));
            SetColor(i, color);
        }
    }
      ...
      Color background)
    {
        ...
        do
        {
            ...
            {
                Color[] pixels = pixelSequence.GetPixels(background);
                if (predicate(pixels))
                {
                    flag = true;
    ...
    private IEnumerable<PixelSequence> GetPixelSequences(
      int sequenceLength)
    {
    ...
    private class PixelSequence
    {
        ...
        {
            return _indices
                .Select(i => i != -1 ? _source.GetColor(i) : background)
                .ToArray();
        }

...
public sealed class ContextMenuPlotter : IContextMenuPlotter
{
        ...
        get
        {
            if (_contextMenuPlacementWindow is null)
            {
                ...
                using (DpiAwareness.EnterDpiScope(DpiAwareness.ProcessDpiAwarenessContext))
                {
                    ...
                    Screen.SetInitialWindowRect(new WindowInteropHelper(_contextMenuPlacementWindow).EnsureHandle(), _contextMenuPlacementWindow, Int32Rect.Empty);
    ...
    private static Point GetContextMenuLocation(ref UIElement? uiElement)
    {
        ...
        return uiElement?.DeviceToLogicalPoint(p) ?? DpiAwareness.PointLogicalPoint(p);
    }

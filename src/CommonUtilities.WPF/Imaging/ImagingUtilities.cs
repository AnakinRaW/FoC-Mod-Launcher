...
internal static class ImagingUtilities
{
    ...
    private static BitmapSource? LoadManagedImage(ImageDefinition imageDefinition, Size deviceSize)
    {
        ...
        try
        {
            switch (FetchResourceStream(uri, out stream))
            {
                case UriResourceType.Unknown:
                    return null;
                case UriResourceType.Png:
                    return LoadBitmappedImage(stream!, deviceSize);
                case UriResourceType.Baml:
                    return LoadXamlImage(stream!, deviceSize);
                case UriResourceType.Xaml:
                    return LoadXamlImage(uri, deviceSize);
            }
        }
        finally
        {
    ...
    private static BitmapSource? LoadXamlImage(object imageAsObject, Size deviceSize)
    {
        ...
        switch (imageAsObject)
        {
            case BitmapSource image:
                return StretchImage(image, deviceSize);
            case FrameworkElement element:
                return RasterizeElement(element, deviceSize);
            default:
                throw new Exception("Unsupported element type. The element must be a UIElement or a BitmapSource.");
        }
    }
    ...
    private static UriResourceType GetResourceType(string contentType)
    {
        ...
        if (lowerInvariant == "application/xaml+xml")
            return UriResourceType.Xaml;
        if (lowerInvariant == "application/baml+xml")
            return UriResourceType.Baml;
        return lowerInvariant == "image/png" ? UriResourceType.Png : UriResourceType.Unknown;
    }


    internal enum UriResourceType
    {
        Unknown,
        Png,
        Baml,
